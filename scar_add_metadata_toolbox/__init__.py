...
class CatalogueServiceWeb(_CatalogueServiceWeb):
    ...
    def _invoke(self, mock_requests_post, mock_requests_request):
        ...
        if hasattr(self, "operations"):
            ...
            try:
                ...
                if isinstance(self.request, str):  # GET KVP
                    ...
                    post_verbs = [
                        x for x in op.methods if x.get("type").lower() == "post"
                    ]
                    if len(post_verbs) > 1:
        ...
        if isinstance(self.request, str):  # GET KVP
            ...
            self.response = http_post(
                request_url, self.request, self.lang, self.timeout, auth=self.auth
            )
    ...
    def getrecords2(
        ...
        if xml is not None:
            ...
            val = self.request.find(
                util.nspath_eval("csw:Query/csw:ElementSetName", csw_namespaces)
            )
            if val is not None:
            ...
            etree.SubElement(
                node1, util.nspath_eval("csw:ElementSetName", csw_namespaces)
            ).text = esn
            ...
            if any([len(constraints) > 0, cql is not None]):
                ...
                # Now add a CQL filter if passed in
                    ...
                    etree.SubElement(
                        node2, util.nspath_eval("csw:CqlText", csw_namespaces)
                    ).text = cql
        ...
        if self.exceptionreport is None:
            ...
            val = self._exml.find(
                util.nspath_eval("csw:SearchResults", csw_namespaces)
            ).attrib.get("numberOfRecordsMatched")
            self.results["matches"] = int(util.testXMLValue(val, True))
            val = self._exml.find(
                util.nspath_eval("csw:SearchResults", csw_namespaces)
            ).attrib.get("numberOfRecordsReturned")
            self.results["returned"] = int(util.testXMLValue(val, True))
            val = self._exml.find(
                util.nspath_eval("csw:SearchResults", csw_namespaces)
            ).attrib.get("nextRecord")
            if val is not None:
...
def create_app():
    ...
    if app.config["APP_ENABLE_FILE_LOGGING"]:
        file_log = RotatingFileHandler(
            app.config["LOG_FILE_PATH"], maxBytes=5242880, backupCount=5
        )
        ...
        try:
            aws_cli(
                [
                    "s3",
                    "sync",
                    str(app.config["STATIC_BUILD_DIR"]),
                    f"s3://{app.config['S3_BUCKET']}",
                    "--delete",
                ]
            )
            echo(
    ...
    def sign_in():
        ...
        auth_flow = app.config["CLIENT_AUTH"].initiate_device_flow(
            scopes=app.config["AUTH_CLIENT_SCOPES"]
        )
        if "user_code" not in auth_flow:
            app.logger.error(
                "Authentication failure - 'user_code' not returned by Azure"
            )
            app.logger.error(json.dumps(auth_flow))
            raise RuntimeError(
                "Could not start authentication flow. No 'user_code' returned by Azure."
            )

        app.logger.info(
            f"Authorisation request successful, displaying user instructions for how to authenticate"
        )
        echo(
        ...
        if "access_token" not in auth_token:
            app.logger.error(
                "Authentication failure - 'access_token' not returned by Azure"
            )
            app.logger.error(json.dumps(auth_flow))
            raise RuntimeError(
                "Could not start authentication flow. No 'access_token' returned by Azure."
            )
        ...
        app.config["APP_AUTH_SESSION_FILE_PATH"].parent.mkdir(
            parents=True, exist_ok=True
        )
        app.logger.info(
            f"Creating auth file as [{app.config['APP_AUTH_SESSION_FILE_PATH'].absolute()}]"
        )
    ...
    def sign_out():
        ...
        click_confirm(
            text=f"{click_style('Are you sure you want to sign-out?', fg='yellow')}",
            abort=True,
        )
        app.logger.info(
    ...
    def list_records():
        ...
        app.logger.info(
            f"Listing records from CSW catalogue [{app.config['CSW_ENDPOINT_UNPUBLISHED']}]"
        )

        csw_auth = CatalogueServiceWebAuthentication(
            token=app.config["CLIENT_AUTH_TOKEN"]["access_token"]
        )
        csw_unpublished = CatalogueServiceWeb(
            app.config["CSW_ENDPOINT_UNPUBLISHED"], auth=csw_auth
        )
        ...
        csw_published = CatalogueServiceWeb(
            app.config["CSW_ENDPOINT_PUBLISHED"], auth=csw_auth
        )
    ...
    def import_record():
        ...
        app.logger.info(
            f"Importing a record as JSON into CSW catalogue [{app.config['CSW_ENDPOINT_UNPUBLISHED']}]"
        )
        ...
        echo(
            f"Importing metadata record from file {click_style(str(options['path'].absolute()), fg='blue')}"
        )
        if not options["path"].exists():
            # TODO: Raise proper error
            app.logger.error(
                f"Unable to import file, path [{str(options['path'].absolute())}], does not exist"
            )
        ...
        options["data"]["date_stamp"] = date.fromisoformat(
            options["data"]["date_stamp"]
        )
        options["data"]["reference_system_info"]["authority"]["dates"][0][
            "date"
        ] = date.fromisoformat(
        ...
        for index, resource_date in enumerate(options["data"]["resource"]["dates"]):
            if (
                "date_precision" in resource_date.keys()
                and resource_date["date_precision"] == "year"
            ):
            ...
            if len(options["data"]["resource"]["dates"][index]["date"]) == 10:
                options["data"]["resource"]["dates"][index][
                    "date"
                ] = date.fromisoformat(
        ...
        for index, keyword in enumerate(options["data"]["resource"]["keywords"]):
            options["data"]["resource"]["keywords"][index]["thesaurus"]["dates"][0][
                "date"
            ] = date.fromisoformat(
                options["data"]["resource"]["keywords"][index]["thesaurus"]["dates"][0][
                    "date"
        ...
        if (
            len(options["data"]["resource"]["extent"]["temporal"]["period"]["start"])
            == 4
        ):
        ...
        if "measures" in options["data"]["resource"].keys():
            options["data"]["resource"]["measures"][0]["dates"][0][
                "date"
            ] = date.fromisoformat(
        ...
        app.logger.info(
            f"Creating record [{options['record']}] (without XML declaration)"
        )
    ...
    def export_record():
        ...
        app.logger.info(
            f"Exporting a record from CSW catalogue [{app.config['CSW_ENDPOINT_UNPUBLISHED']}] as JSON"
        )
        ...
        for record in csw.records:
            records[csw.records[record].identifier] = csw.records[
                record
            ].identification.title
        _records = {
            f"* {click_style(k, fg='cyan')} - {click_style(v, fg='magenta')}": k
        ...
        selected_record = prompt(
            [
                inquirer_list(
                    name="record",
                    message="Which record would like to export?",
                    choices=list(_records.items()),
                ),
            ]
        )
        ...
        app.logger.info(
            f"selected record [{options['record']}], selected path [{options['path']}]"
        )
        ...
        csw = CatalogueServiceWeb(app.config["CSW_ENDPOINT_UNPUBLISHED"], auth=csw_auth)
        csw.getrecordbyid(
            id=[options["record"]], outputschema="http://www.isotc211.org/2005/gmd"
        )
        if len(csw.records) != 1:
            # TODO: Add error
            raise RuntimeError(
                f"No record with identifier [{options['record']}] exists in CSW catalogue"
            )
        ...
        if not options["path"].exists():
            # TODO: Add error
            raise RuntimeError(
                f"specified export path [{options['path']}] does not exist"
            )
    ...
    def remove_record():
        ...
        selected_record = prompt(
            [
                inquirer_list(
                    name="record",
                    message="Which record would like to remove?",
                    choices=list(_records.items()),
                ),
            ]
        )
        ...
        echo(
            f"{click_style('Successfully removed', fg='green')} record [{click_style(options['record'], fg='cyan')}]."
        )
    ...
    def publish_record():
        ...
        selected_record = prompt(
            [
                inquirer_list(
                    name="record",
                    message="Which record would like to publish?",
                    choices=list(_records.items()),
                ),
            ]
        )
    ...
    def retract_record():
        ...
        selected_record = prompt(
            [
                inquirer_list(
                    name="record",
                    message="Which record would like to retract?",
                    choices=list(_records.items()),
                ),
            ]
        )
    ...
    def view_collection(collection_id: str):
        ...
        collection["description"] = markdown(
            collection["description"], output_format="html5"
        )

        # TODO: Get collection items via a CSW search for a keyword - for now include all items
        collection["items"] = []
        app.logger.info(
            f"Requesting all records from CSW endpoint [{app.config['CSW_ENDPOINT_PUBLISHED']}]"
        )
        ...
        for record in csw.records.values():
            ...
            for index, contact in enumerate(item["resource"]["contacts"]):
                if "organisation" in contact and "name" in contact["organisation"]:
                    contact["organisation"]["display_name"] = contact["organisation"][
                        "name"
                    ]
                    if (
            ...
            for item_contact in item["resource"]["contacts"]:
                for item_contact_role in item_contact["role"]:
                    if item_contact_role not in list(
                        item["resource"]["contacts_by_role"].keys()
                    ):
                        item["resource"]["contacts_by_role"][item_contact_role] = []
                    item["resource"]["contacts_by_role"][item_contact_role].append(
                        item_contact
                    )
    ...
    def view_item(item_id: str):
        ...
        csw = CatalogueServiceWeb(app.config["CSW_ENDPOINT_PUBLISHED"])
        app.logger.info(
            f"Requesting record [{item_id}] from CSW endpoint [{app.config['CSW_ENDPOINT_PUBLISHED']}]"
        )
        ...
        item["resource"]["extent"]["temporal"]["period"]["start"] = {
            ...
            "display_value": item["resource"]["extent"]["temporal"]["period"][
                "start"
            ].isoformat(),
        }
        ...
        if item["resource"]["maintenance"]["maintenance_frequency"] == "biannually":
            item["resource"]["maintenance"]["status"] = "overdue"
            next_release_expected = item["resource"]["dates_by_type"]["released"][
                "date"
        ...
        for item_constraint in item["resource"]["constraints"]["usage"]:
            if "copyright_licence" in item_constraint:
                # https://gitlab.data.bas.ac.uk/uk-pdc/metadata-infrastructure/metadata-standards/issues/150 workaround
                if (
                    item_constraint["copyright_licence"]["href"]
                ...
                if (
                    ...
                    == "This information is licensed under the Create Commons Attribution 4.0 International Licence "
                    "(CC BY 4.0). To view this licence, visit https://creativecommons.org/licenses/by/4.0/"
                ):
                    item_constraint["copyright_licence"]["reformat_statement"] = True
                item["resource"]["constraints"]["copyright"] = item_constraint[
                    "copyright_licence"
                ]
            ...
            if "required_citation" in item_constraint:
                ...
                item["resource"]["constraints"]["citation"] = item_constraint[
                    "required_citation"
                ]
                ...
                if (
                    ...
                    and item["file_identifier"]
                    == "86bd7a1a-845d-48a9-8d71-59fdf7290556"
                ):
                    ...
                    item["resource"]["constraints"]["citation"][
                        "elements"
                    ] = cite_proc.json
        ...
        for index, keywords in enumerate(item["resource"]["keywords"]):
            if (
                ...
                if (
                    ...
                    item["resource"]["special_keywords"]["topic"] = {
                        "display_value": keywords["terms"][0]["term"]
                    }
                elif (
                    keywords["thesaurus"]["title"]["href"]
        ...
        for index_to_remove in sorted(
            descriptive_keywords_indexes_to_remove, reverse=True
        ):
            item["resource"]["keywords"].pop(index_to_remove)

        item["resource"]["keywords_by_type"] = {}
        for item_keywords in item["resource"]["keywords"]:
            if item_keywords["type"] not in list(
                item["resource"]["keywords_by_type"].keys()
            ):
                item["resource"]["keywords_by_type"][item_keywords["type"]] = []
            item["resource"]["keywords_by_type"][item_keywords["type"]].append(
                item_keywords
            )
        ...
        item["metadata_links"] = {
            "iso_xml": url_for(
                ".view_record",
                record_format="iso-xml",
                record_id=item["file_identifier"],
            ),
            "iso_html": url_for(
                ".view_record",
                record_format="iso-html",
                record_id=item["file_identifier"],
            ),
            "iso_rubric": url_for(
                ".view_record",
                record_format="iso-rubric",
                record_id=item["file_identifier"],
            ),
        }
        ...
        for index, transfer_option in enumerate(item["resource"]["transfer_options"]):
            ...
            if (
                "online_resource" in transfer_option
                and "title" in transfer_option["online_resource"]
            ):
                if transfer_option["online_resource"]["title"] == "GeoPackage":
                    ...
                    transfer_option["online_resource"]["title"]
                    == "Web Map Service (WMS)"
                ):
                    ...
                    if (
                        "layer" in query_string_parsed
                        and len(query_string_parsed["layer"]) == 1
                    ):
                        transfer_option["format"]["layer"] = query_string_parsed[
                            "layer"
                        ][0]
    ...
    def view_item():
        # TODO: Make dynamic
        yield {"item_id": "86bd7a1a-845d-48a9-8d71-59fdf7290556"}
    ...
    def view_record(
        ...
        app.logger.info(
            f"Requesting record [{record_id}] from CSW endpoint [{app.config['CSW_ENDPOINT_PUBLISHED']}]"
        )
        csw = CatalogueServiceWeb(app.config["CSW_ENDPOINT_PUBLISHED"])
        csw.getrecordbyid(
            id=[record_id], outputschema="http://www.isotc211.org/2005/gmd"
        )
        if len(csw.records) != 1:
            # TODO: Add specific error
            # noinspection PyUnresolvedReferences
        ...
        if record_format == "iso-xml":
            ...
            return Response(
                record_xml, mimetype="text/xml", content_type="text/xml; charset=utf-8"
            )
        elif record_format == "iso-html":
            app.logger.info(
                f"Return CSW response as ISO HTML (NOAA ISO XML to HTML stylesheet)"
            )
            ...
            record_iso_html_root.addprevious(
                ProcessingInstruction(
                    "xml-stylesheet",
                    'type="text/xsl" href="/static/xsl/iso-html/xml-to-html-ISO.xsl"',
                )
            )
            record_iso_html_xml = tostring(
                record_iso_html,
                pretty_print=True,
            ...
            app.logger.info(
                f"Return CSW response as ISO Rubric (NOAA ISO SpiralTracker Rubric stylesheet)"
            )
            ...
            record_iso_rubric_root.addprevious(
                ProcessingInstruction(
                    "xml-stylesheet",
                    'type="text/xsl" href="/static/xsl/iso-rubric/isoRubricHTML.xsl"',
                )
            )
    ...
    def view_record():
        # TODO: Make dynamic
        yield {
            "record_id": "86bd7a1a-845d-48a9-8d71-59fdf7290556",
            "record_format": "iso-xml",
        }
        yield {
            "record_id": "86bd7a1a-845d-48a9-8d71-59fdf7290556",
    ...
    def csw_server_unpublished():
        ...
        csw = Csw(
            rtconfig=app.config["CSW_CONFIG_UNPUBLISHED"],
            env=request.environ,
            version="2.0.2",
        )
    ...
    def csw_server_published():
        ...
        csw = Csw(
            rtconfig=app.config["CSW_CONFIG_PUBLISHED"],
            env=request.environ,
            version="2.0.2",
        )
        if request.method == "GET":
